    ...
    public function check(string $data)
    {
        try {
            ...
            foreach ($this->selector as $element) {
                $elements[] = (new EurasianBankChecker())->getDocument($data, $element);
            }
            ...
            for ($i = 0; $i < count($currency_info); $i += 3) {
                ...
                if (!(is_numeric($currency_info[$i + 1]) && is_numeric($currency_info[$i + 2]))) {
                    throw new \Exception('Евразийский банк. Значение валют не числовой');
                }
            }

            if (!$count) {
                throw new \Exception('Евразийский банк. Структура сайта изменилась либо нету всех данных');
            }
